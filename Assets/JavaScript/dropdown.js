...
function buildDropDown(values) {
    ...
    // contents.push(`<a class="dropdown-item" href="#" onclick="loadNewChorale("./${name}", 72)">${name.substring(20, name.length-4)}</a>`)
    contents.push(`<input type="button" id="${name}" class="dropdown-item" type="button" value="${name.substring(20, name.length - 4)}"  `) //no closing bracket leave room for style
            if(irregularChorales.includes(name)){
    contents.push(` style="background-color: lightyellow;" />`);
    } else {
        contents.push(`/>`);
    }
    counter++;
...
//If the user clicks on any item, call loadNewChorale 
$(`#menuItems`).on('click keypress', '.dropdown-item', function (event) {
