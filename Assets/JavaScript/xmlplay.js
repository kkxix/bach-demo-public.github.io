//~ Revision: 93, Copyright (C) 2016-2018: Willem Vree, contributions Stéphane David.
...
function readAbcOrXML(abctxt) {
    ...
    var options = { 'p': 'f', 't': 1, 'u': 0, 'v': 3, 'm': 2 }; // t==1 -> tab en perc naar %%map
    if (noPF) options.p = '';
    if (noLB) options.x = 1;
...
// Entry point
function readLocalFile(url, audioContext) {
    fileURL = url;
    ...
    request.onload = function () {
        freader.readAsText(request.response);
    };
    request.send();

    playbtn = document.getElementById('pause');
...
function doLayout(abctxt) {
    ...
    abcElm.innerHTML = '<div id="leeg" style="height:10px">&nbsp;</div>';
...
function putMarkLoc(n) { // function that moves the rectangles
...
function mkNtsSeq() {           //make note sequence 
    var curNoteTime = iSeq > 0 ? ntsSeq[iSeq].t : 0;
    ntsSeq = []; barTimes = {};
    var repcnt = 1, offset = 0, repstart = 0, reptime = 0, volta = 0, tvolta = 0, i, n;
    for (i = 0; i < allNotes.length; ++i) {     //for each note
        n = allNotes[i];        //one note 
        if (n.bt && n.v == 0) {
            console.log('Reached n.bt && n.v == 0 case')
    ...
    console.log(ntsSeq.length);
    console.log(ntsSeq);
    if (iSeq == ntsSeq.length) iSeq -= 1;
    putMarkLoc(ntsSeq[iSeq]);   //put the rectangle on the first note before playing 
}

function changeCurTime(newTime) {
    currentSongTime = newTime;
    newTime *= 1000;        //convert to miliseconds 
    var accumTime = 0;
    iSeq = 0;

    var tfac = 60000 / 384;
    var tf = tfac / (curTemp * tempScale);

    //compute iSeq based on newTime ...
    while (accumTime < newTime) {

        //calculate dt 
        var t1 = ntsSeq[iSeq + 1].t;       // abc time of next note 
        var dt = (t1 - ntsSeq[iSeq].t) * tf;          // delta abc time * tf = delta real time in msec
